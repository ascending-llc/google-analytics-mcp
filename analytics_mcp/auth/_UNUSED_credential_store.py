"""
Credential Store for Analytics MCP

Provides a standardized interface for credential storage and retrieval,
supporting file-based storage for per-user OAuth credentials.
"""
...
class LocalDirectoryCredentialStore(CredentialStore):
    ...
    def __init__(self, base_dir: Optional[str] = None):
        ...
        Args:
            base_dir: Base directory for credential files. If None, uses
                     ~/.analytics_mcp/credentials as default.
        """
        if base_dir is None:
            if os.getenv("GOOGLE_MCP_CREDENTIALS_DIR"):
                ...
                if home_dir and home_dir != "~":
                    base_dir = os.path.join(
                        home_dir, ".analytics_mcp", "credentials"
                    )
                else:
    ...
    def _get_credential_path(self, user_email: str) -> str:
        ...
        if not os.path.exists(self.base_dir):
            os.makedirs(self.base_dir, mode=0o700)  # Secure permissions
    ...
    def store_credential(self, user_email: str, credentials: Credentials) -> bool:
        ...
        try:
            ...
            os.umask(0o077)  # Ensure restrictive default
            with open(creds_path, "w") as f:
                json.dump(creds_data, f, indent=2)
            os.chmod(creds_path, 0o600)  # Explicitly set secure permissions
...
def get_credential_store() -> CredentialStore:
    ...
    if _credential_store is None:
        # Always use LocalDirectoryCredentialStore as the default
